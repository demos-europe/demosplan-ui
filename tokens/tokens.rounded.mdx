...
<Table
  ...
  rows={[
    ...
    ["$dp-rounded-lg", "rounded-lg", "Large - The biggest radius. Use for boxes that purposefully should appear rounded - plays nice with rounded elements inside it that are rounded via `rounded.sm`."],
    ...
    ["$dp-rounded-sm", "rounded-sm", "Default - The standard for UI elements that shall appear rounded. Use this value for form elements and other smaller UI elements. When in doubt, pick this one."],
    ["$dp-rounded-xs", "rounded-xs", "Smallest available radius discernible from 0. Use this value for rounded elements inside elements that have `rounded.sm` applied as border-radius."],
